import {JPA_API_URL} from "../common/constants";

export const updateWidget = async (widgetId, widget) => {
    const response = await fetch(`${JPA_API_URL}/widgets/${widgetId}`, {
        method: 'PUT',
...
{
    const response = await fetch(`${JPA_API_URL}/topics/${topicId}/widgets`, {
        method: "POST",
        body: JSON.stringify(widget),
        headers: {
            'content-type': 'application/json'
...
export const updateWidgetOrder = async(widgetId, widget, direction) => {
    const response = await fetch(`${JPA_API_URL}/widgets/${widgetId}/${direction}`, {
            method: 'PUT',
            body: JSON.stringify(widget),
            headers: {
                'content-type': 'application/json'
