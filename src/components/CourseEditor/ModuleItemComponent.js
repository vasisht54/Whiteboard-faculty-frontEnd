...
class ModuleItemComponent extends React.Component {
  ...
  render() {
    const moduleItemClass = `nav-link list-group-item my-2 font-weight-bolder wbdv-module-item-title${
      this.props.activeModuleId === this.state.module._id ? " bg-primary" : ""
    }`;
    return (
      <li className="nav-item bg-dark pl-3 wbdv-module-item">
        {!this.state.editing ? (
          <div className={moduleItemClass}>
            ...
            {this.props.activeModuleId === this.state.module._id && (
              <i
                onClick={() =>
