...
const dispatchToPropertyMapper = (dispatch) => {
  return {
    ...
    deleteTopic: (topicId) => {
      topicService
        .deleteTopic(topicId)
        .then((response) => dispatch(deleteTopic(topicId)));
    },
    createTopic: (lessonId, topic) => {
      topicService
        .createTopic(lessonId, topic)
        .then((response) => dispatch(createTopic(response)));
    },
    editTopic: (topic) => {
      topicService.updateTopic(topic._id, topic).then((response) =>
        topicService
          .findTopicsForLesson(topic.lessonId)
          .then((actualTopics) => {
