...
class TopicPillComponent extends React.Component {
  ...
  render() {
    const topicClass = `nav-link wbdv-topic${
      this.props.activeTopicId === this.state.topic._id.toString()
        ? " shadow"
        : ""
    }`;
    return (
        ...
        {!this.state.editing ? (
          <div>
            <div className={topicClass}>
              <Link
                onClick={this.findWidgets}
                to={`/course-editor/${this.props.courseId}/modules/${this.props.moduleId}/lessons/${this.props.lessonId}/topics/${this.state.topic._id}`}
              >
